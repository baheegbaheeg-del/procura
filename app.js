(function(){
  'use strict';
  var DB_KEY='silah_final_v3_db';
  var LEGACY_DB_KEY='silah_final_v2_db';
  var SESSION_KEY='silah_final_v2_session';
  var FILE_DB='silah_final_v3_files';
  var db=loadDB();
  ...
  function loadDB(){try{return normalize(JSON.parse(localStorage.getItem(DB_KEY)||localStorage.getItem(LEGACY_DB_KEY)))}catch(e){return baseDB()}}
  ...
  function cloudMessage(err){return (err&&err.message)||'تعذر حفظ التعديل في قاعدة البيانات. تحقق من الاتصال ثم حاول مرة أخرى.'}
  ...
  function demoAttachment(name,text){var content=String(text||'مستند تجريبي في منصة صِلَة.');return {id:'demo-'+name.replace(/[^a-z0-9]/gi,'').slice(0,20)+'-'+Math.random().toString(36).slice(2,7),name:name,size:content.length,type:'text/plain',dataUrl:'data:text/plain;charset=utf-8,'+encodeURIComponent(content),createdAt:now()}}
  ...
  function demoData(){
    ...
    d.profiles.forEach(function(p){
      ...
      p.documentData={commercialNumber:p.role==='buyer'?'15684':'42197',taxNumber:p.role==='buyer'?'712-458-903':'553-204-618',signerName:p.contact,signerTitle:p.role==='buyer'?'مدير المشتريات':'المدير المفوض',commercialFile:demoAttachment('سجل-تجاري-تجريبي.txt','سجل تجاري تجريبي — '+p.company),taxFile:demoAttachment('بطاقة-ضريبية-تجريبية.txt','بطاقة ضريبية تجريبية — '+p.company),signerFile:demoAttachment('تفويض-توقيع-تجريبي.txt','تفويض توقيع تجريبي — '+p.contact)};
    });
    var r1={id:'R1',number:'RFQ-00001',buyerEmail:'buyer@silah.demo',company:'شركة النور للتشطيبات',project:'تشطيب فيلا — التجمع الخامس',category:'سباكة ومواسير PPR',location:'التجمع الخامس — القاهرة الجديدة',deadline:new Date(t+5*86400000).toISOString().slice(0,10),payment:'أجل 30 يوم',notes:'يرجى تقديم الماركة وبلد المنشأ والضمان. التوريد على دفعتين.',attachments:[demoAttachment('BOQ-فيلا-التجمع.txt','ملف BOQ تجريبي لمشروع فيلا التجمع.'),demoAttachment('مواصفات-فنية.txt','المواصفات الفنية التجريبية للطلب.')],status:'مفتوح لاستقبال العروض',createdAt:new Date(t-86400000).toISOString(),items:[{id:'I101',name:'مواسير PPR 20 مم',qty:300,unit:'متر',spec:'ضغط 20 بار'},{id:'I102',name:'مواسير PPR 25 مم',qty:200,unit:'متر',spec:'ضغط 20 بار'},{id:'I103',name:'كوع PPR 20 مم',qty:100,unit:'قطعة',spec:'مساوٍ'}]};
    var r2={id:'R2',number:'RFQ-00002',buyerEmail:'buyer@silah.demo',company:'شركة النور للتشطيبات',project:'مشروع إداري — مصر الجديدة',category:'سباكة ومواسير PPR',location:'مصر الجديدة',deadline:new Date(t-9*86400000).toISOString().slice(0,10),payment:'نقدي',notes:'طلب عرض خاص بمشروع إداري.',attachments:[],status:'تم إصدار أوامر شراء',createdAt:new Date(t-12*86400000).toISOString(),items:[{id:'I201',name:'محبس كرة 1 بوصة',qty:20,unit:'قطعة',spec:'نحاس'},{id:'I202',name:'وصلة PPR 25 مم',qty:60,unit:'قطعة',spec:'حراري'}]};
    d.rfqs=[r1,r2];
    d.quotes=[
  ...
  window.showLanding=function(){window.logout()}
  ...
  $('loginForm').addEventListener('submit',async function(e){
    e.preventDefault();var email=$('email').value.trim().toLowerCase(),pass=$('password').value,role=$('role').value;if(!email||pass.length<4)return;
    if(!cloudConfigured()){
      var p=profile(email,role);if(!p){p={id:uid('PR'),email:email,role:role,name:email.split('@')[0],company:'',contact:'',phone:'',city:'القاهرة',categories:[],brands:'',paymentTerms:'نقدي',leadTime:3,creditLimit:role==='buyer'?0:null,creditRestricted:false,verification:'غير مكتمل',docs:{commercial:false,tax:false,signer:false},rating:0,financial:0,operational:0,founder:role==='supplier',createdAt:now()};db.profiles.push(p);addLog('تم إنشاء حساب '+roleName(role)+' جديد')}
      session={email:email,role:role,cloud:false};localStorage.setItem(SESSION_KEY,JSON.stringify(session));active=complete(p)?'dashboard':'profile';save();if(!complete(p))flash('أكمل ملف الشركة أولًا ليظهر حسابك بصورة احترافية');return;
    }
    try{
      var authData;
      try{authData=await window.SilahCloud.signIn(email,pass)}catch(loginError){
        if(role==='admin')throw loginError;
        authData=await window.SilahCloud.signUp(email,pass,role);
        if(!authData.session){flash('تم إنشاء الحساب. راجع بريدك لتأكيده ثم سجّل الدخول.');return}
      }
      var user=authData.user||(authData.session&&authData.session.user);if(!user)throw new Error('تعذر استكمال جلسة الدخول.');
      var remoteProfile=await window.SilahCloud.getMyProfile();if(!remoteProfile)throw new Error('تم إنشاء الحساب، لكن ملفه لم يجهز بعد. أعد المحاولة بعد لحظات.');
      if(remoteProfile.role!==role)throw new Error('نوع الحساب المختار لا يطابق نوع هذا البريد.');
      ...
      var current=me();if(current&&!complete(current)){active='profile';render();flash('أكمل ملف الشركة أولًا ليظهر حسابك بصورة احترافية')}else flash('تم تسجيل الدخول وربط الحساب بقاعدة البيانات.');
    }catch(err){flash('تعذر تسجيل الدخول: '+cloudMessage(err))}
  });
  ...
  window.logout=async function(){var wasCloud=usingCloud();if(wasCloud)window.SilahCloud.unsubscribe();session=null;localStorage.removeItem(SESSION_KEY);render();if(wasCloud){try{await window.SilahCloud.signOut()}catch(e){}}}
  ...
  window.addEventListener('storage',function(e){if(e.key===DB_KEY||e.key===LEGACY_DB_KEY){db=loadDB();if(session)render();}});
